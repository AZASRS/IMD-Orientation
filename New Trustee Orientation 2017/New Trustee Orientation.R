...
load('../R projects/private investment performance/pmedata.rdata')
load("../R projects/Public Performance/Scripts/public.data.rdata")
source('../R projects/basic financial.r',echo=FALSE)
effective.date = "2017-06-30"
label.d = "as of 6/30/17"
label.pms = "as Reported 6/30/17"
tomillions = function(x) x/1000000
nepc=read.csv('../R projects/Total Fund Value Add/data/nepc data.csv', stringsAsFactors = FALSE)
nepc$Date = as.Date(nepc$Date,format="%m/%d/%Y")
comp.mgr.dat = read.csv("../R projects/Total Fund Value Add/Data/NEPC Mgr Data revised.csv", stringsAsFactors = FALSE)
comp.mgr.dat$Date = as.Date(comp.mgr.dat$AsOf, format = '%m/%d/%Y')

## @knitr saa
saa <- read.csv('../R projects/Total Fund Active Weights/SAA.changes.csv',stringsAsFactors = F)
...
#private returns
priv.ret = read.csv("../R projects/Total Fund Value Add/Data/private returns.csv")
priv.ret = priv.ret[,-1]
...
dvadf=rbind(dvadf,df.i)
#add Total Privates
lookback=read.csv("../R projects/Total Fund Value Add/Data/lookback private add value.csv")
